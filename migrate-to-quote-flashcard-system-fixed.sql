...
-- This script safely migrates existing data and creates the new comprehensive system
-- FIXED VERSION: Uses TEXT for book_id to match existing books table
...
CREATE TABLE public.quotes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    book_id TEXT REFERENCES public.books(id) ON DELETE CASCADE NOT NULL, -- FIXED: TEXT not UUID
...
CREATE TABLE public.student_flashcard_sets (
    ...
    description TEXT,
    book_id TEXT REFERENCES public.books(id) ON DELETE CASCADE, -- FIXED: TEXT not UUID
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Updated timestamp triggers (use existing function)
...
BEGIN
    ...
    IF backup_exists THEN
        ...
        -- Migrate old quotes to new structure
        ...
        RAISE NOTICE 'Successfully migrated data from old quotes table';
    END IF;
END $$;
